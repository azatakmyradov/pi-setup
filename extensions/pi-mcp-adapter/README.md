...
Successful manual or on-demand connections are remembered per project when `lifecycle` is omitted. The server is treated as `keep-alive` the next time Pi opens that project. Run `/mcp disconnect <server>` or `mcp({ disconnect: "server" })` to disconnect it and clear the remembered state. An explicit `lifecycle: "lazy"` disables remembered auto-connect, while explicit `eager` and `keep-alive` settings remain authoritative.
