...
export const invokeCoercion = (ref: CoercionFunction, args: Array<unknown>, node: AstNode): unknown => {
  ...
  if (ref.name === "parseInt") {
    ...
    return parseInt(coerceToString(value), radix as number | undefined)
  }
  if (ref.name === "parseFloat") return parseFloat(coerceToString(value))
  return coerceToString(value)
}
import { type AstNode, CoercionFunction, InterpreterRuntimeError } from "../interpreter/model.ts"
import { copyIn, type SafeObject } from "../tool-runtime.ts"
import {
  isSandboxValue,
  SandboxDate,
  SandboxMap,
