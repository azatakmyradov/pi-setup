...
const mcpResultSummarySchema = z.object({
  omitted: z.literal(true),
  isError: z.boolean(),
  contentBlocks: z.number(),
  fullResultPath: z.string().optional(),
  structuredContent: z.object({ omitted: z.literal(true) }).optional(),
});
...
describe("guardMcpOutput", () => {
  ...
  it("truncates large text output and saves the full output to a file", async () => {
    ...
    const fullOutputPath = guarded.outputGuard?.fullOutputPath;
    if (!fullOutputPath) throw new Error("expected the full output artifact path");
    const saved = await readFile(fullOutputPath, "utf8");
    expect(saved).toBe(text);

    const summary = mcpResultSummarySchema.parse(guarded.mcpResult);
  ...
  it("summarizes details.mcpResult only when it exceeds detailsMaxBytes", async () => {
    ...
    const summary = mcpResultSummarySchema.parse(summarized.mcpResult);
