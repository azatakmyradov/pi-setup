...
export async function lazyConnect(state: McpExtensionState, serverName: string, signal?: AbortSignal, remember = true): Promise<boolean> {
  const connection = state.manager.getConnection(serverName);
  if (connection?.status === "needs-auth") {
    return false;
  }
  if (connection?.status === "connected") {
    updateServerMetadata(state, serverName);
    if (remember) rememberConnectedServer(state, serverName);
    return true;
  }
...
export function rememberConnectedServer(state: McpExtensionState, serverName: string): void {
  const definition = state.config.mcpServers[serverName];
  if (!definition || !state.projectCwd || definition.lifecycle === "lazy" || definition.lifecycle === "eager") return;
