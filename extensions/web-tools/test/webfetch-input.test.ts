...
test("parseWebFetchToolParams rejects invalid boundary input", () => {
  ...
  assert.equal(
    parseWebFetchToolParams({ url: "ftp://example.com" }, testFetchSettings)._tag,
    "err",
  );
  assert.deepEqual(
    parseWebFetchToolParams({ url: "https://example.com", format: "pdf" }, testFetchSettings),
    {
      _tag: "err",
      error: {
  ...
  assert.deepEqual(
    parseWebFetchToolParams({ url: "https://example.com", depth: "auto" }, testFetchSettings),
    {
      _tag: "err",
  ...
  assert.deepEqual(
    parseWebFetchToolParams({ url: "https://example.com", timeout: "30" }, testFetchSettings),
    {
      _tag: "err",
...
test("parseWebFetchToolParams clamps timeout to supported bounds", () => {
  const low = parseWebFetchToolParams(
    { url: "https://example.com", timeout: 0 },
    testFetchSettings,
  );
  const high = parseWebFetchToolParams(
