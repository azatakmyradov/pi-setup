...
test("parseWebSearchToolParams accepts deep and clamps maxResults", () => {
  const low = parseWebSearchToolParams(
    { query: "example", maxResults: 0, depth: "deep" },
    testSearchSettings,
  );
...
test("parseWebSearchToolParams rejects invalid boundary input", () => {
  ...
  assert.deepEqual(
    parseWebSearchToolParams({ query: "example", depth: "slow" }, testSearchSettings),
    {
      _tag: "err",
      error: {
  ...
  assert.deepEqual(
    parseWebSearchToolParams({ query: "example", maxResults: "8" }, testSearchSettings),
    {
      _tag: "err",
