...
describe("question conditions", () => {
  it("shows only roots until a listed parent option is selected", () => {
    ...
    expect(getActiveQuestionIndices(questions, [unanswered(), unanswered(), unanswered()])).toEqual(
      [0],
    );
    expect(getActiveQuestionIndices(questions, [selected(1), unanswered(), unanswered()])).toEqual([
      0, 1,
    ]);
    expect(getActiveQuestionIndices(questions, [selected(2), unanswered(), unanswered()])).toEqual([
      0, 2,
    ]);
  });
  ...
  it("matches any configured option for multi-select parents", () => {
    ...
    expect(
      getActiveQuestionIndices(questions, [selected(2), unanswered(), unanswered()]),
    ).toContain(1);
  });
  ...
  it("supports nested conditions without activating descendants of hidden parents", () => {
    ...
    expect(getActiveQuestionIndices(questions, [selected(1), selected(1), unanswered()])).toEqual([
      0, 1, 2,
    ]);
    expect(getActiveQuestionIndices(questions, [selected(2), selected(1), unanswered()])).toEqual([
      0,
    ]);
  });
  ...
  it("rejects forward and out-of-range references", () => {
    ...
    expect(() => validateQuestionConditions(forward)).toThrow("must reference an earlier question");
    ...
    expect(() => validateQuestionConditions(invalidOption)).toThrow("has 2 options");
  });
});
